...
# ==========================
BOT_TOKENS3 = [
    "8631397367:AAFd7-dglCO7pspynleLBK29PYNf23fD_10",
    "8704564645:AAEAM-d4cLjqb0mV7sVyvluORk_icMWTeGE" # <--- Please put your second bot token here!
]
USERNAME3 = "Smart01"
PASSWORD3 = "Smart01"
BASE3 = "http://2.59.169.96"
TELEGRAM_CHAT_ID3 = "-1003713446342"
...
def format_item3(it):
    ...
    FLAG_IDS = {
        ...
        'TW': '5431596620782354186', 'TJ': '5433836870862331565', 'TZ': '5431596468408987157',
    ...
    for name, (code, f) in country_map.items():
        if name.lower() in country_name_raw.lower() or country_name_raw.lower() in name.lower():
            country_code, flag = (code, f)
            break

    if len(number_raw) >= 7:
        formatted_num = f"{number_raw[:3]}RSF{number_raw[-4:]}"
    else:
        formatted_num = number_raw

    safe_num = html.escape(formatted_num)
    safe_platform = html.escape(platform_raw)

    if "WHATSAPP" in platform_raw or "WS" in platform_raw:
        logo = '<tg-emoji emoji-id="5393189591773630465">💬</tg-emoji>'
    elif "INSTAGRAM" in platform_raw or "IG" in platform_raw:
        logo = '<tg-emoji emoji-id="5319160079465857105">📸</tg-emoji>'
    elif "TIKTOK" in platform_raw or "TT" in platform_raw:
        logo = '<tg-emoji emoji-id="5393189789342123585">🔹</tg-emoji>'    
    elif "TELEGRAM" in platform_raw or "TG" in platform_raw:
        logo = '<tg-emoji emoji-id="5267139126339084336">🔹</tg-emoji>'
    elif "FACEBOOK" in platform_raw or "FB" in platform_raw:
    ...
    return f"{flag_html} #{country_code} {logo} <code>{safe_num}</code>"

# ==========================
# DELETE MESSAGE LATER
# ==========================
def delete_message_later3(bot_token, chat_id, message_id, delay=180):
    time.sleep(delay)
    try:
        requests.post(
...
def telegram_queue_worker3():
    while True:
        try:
            ...
            time.sleep(0.05) # Rapid send; let 429 API status natively throttle us
        except Exception as e:
...
def send_sms_to_telegram3(row):
    try:
        ...
        if otp and otp != "N/A":
            otp_button = [{"text": f" {otp}", "copy_text": {"text": otp}, "style": "primary", "icon_custom_emoji_id": "5296369303661067030"}]
        else:
            otp_button = [{"text": " Waiting for OTP...", "callback_data": "no_otp", "style": "primary", "icon_custom_emoji_id": "5296369303661067030"}]

        buttons = {
            "inline_keyboard": [
                otp_button,
                [
                    {"text": " NUMBER", "url": "https://t.me/Flay_SMS_bot", "style": "success", "icon_custom_emoji_id": "5251203410396458957"},
                    {"text": " CHANNEL", "url": "https://t.me/+bB9mYnCZJ1BkZmNl", "style": "danger", "icon_custom_emoji_id": "5363999757079429238"}
                ]
            ]
        }
        ...
        attempts = 0
        while attempts < 10:
            # Pick a random bot token for each attempt to bypass rate limits
                ...
                if r.status_code == 200:
                    ...
                    if resp_json.get("ok"):
                        ...
                        threading.Thread(
                            ...
                            args=(bot_token, TELEGRAM_CHAT_ID3, message_id, 180),
                            daemon=True
                        ).start()
                        return True
                    else:
                        ...
                        print(f"[Bot3] ❌ Telegram API error: {desc}")
                        if "copy_text" in str(desc):
                            payload["reply_markup"]["inline_keyboard"][0] = [{"text": f" OTP: {otp}", "callback_data": f"otp_{otp}", "style": "primary", "icon_custom_emoji_id": "5330115548900501467"}]
                            continue
                        time.sleep(2)
                        continue
                elif r.status_code == 429:
                    print(f"[Bot3] ⚠ Flood control... Trying again in 2s (Bypassing long waits!)")
                    time.sleep(2)
                    continue # Do NOT increment attempts; try again forever until success
                else:
                    attempts += 1
                    print(f"[Bot3] ❌ Telegram Error (attempt {attempts}):", r.text[:200])
                    time.sleep(2)
            except Exception as e:
                attempts += 1
                print(f"[Bot3] ❌ Send attempt {attempts} error: {e}")
                time.sleep(2)
...
def fetch_sms3():
    ...
    try:
        # Panel operates in UTC+2 timezone
        ...
        if r.status_code >= 500:
            ...
            session3 = requests.Session()  # Completely reset HTTP session
            LOGGED_IN_3 = False
            return None
...
def run_bot3():
    ...
    # Start 5 parallel queue workers to process sends fast without bottlenecking
    for i in range(5):
    ...
    while True:
        try:
            ...
            # Debug: show number of records fetched
            print(f"[Bot3] 📊 Fetched {len(records)} records from panel")
            print("[Bot3] 📦 Raw records:", records[:3])
            valid_rows = []
            for row in records:
                try:
                    if isinstance(row, list) and len(row) >= 6:
                        ...
                        # Support dict format (e.g., API changes)
            ...
            # Initial sync: mark ALL as seen, ALWAYS send last 1
            if not INITIAL_SYNC_DONE_3:
                print("[Bot3] 🔄 Initial sync running...")
                ...
                if send_sms_to_telegram3(recent_item):
                    print(f"[Bot3] ✅ Startup OTP sent → {recent_item['number']}")
                else:
                    print(f"[Bot3] ⚠ Startup OTP send failed → {recent_item['number']}")

                INITIAL_SYNC_DONE_3 = True
                print(f"[Bot3] ✅ Initial sync complete — {len(valid_rows)} items registered, 1 startup OTP sent")

            # Normal mode: only new OTPs
            else:
                new_found = False
                for item in valid_rows:
                    uid = make_uid3(item['number'], item['sms'])
                    if uid not in processed_ids3:
                        print(f"[Bot3] 🆕 Queueing OTP → {item['number']}")
            ...
            # 24-hour reset
            if datetime.now() - START_TIME_3 > timedelta(hours=24):
                try:
                    with file_lock:
                        if os.path.exists(PROCESSED_FILE3):
                            os.remove(PROCESSED_FILE3)
                    processed_ids3.clear()
                    START_TIME_3 = datetime.now()
                    print("[Bot3] 🗑 24-hour reset completed")
                except Exception as e:
                    print(f"[Bot3] ❌ Reset error: {e}")
